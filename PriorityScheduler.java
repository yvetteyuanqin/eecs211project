...
public class PriorityScheduler extends Scheduler {
	...
	protected class ThreadState {
		...
		public void ChangePriority(){
			...
			Iterator<PriorityQueue> myiterator =this.HoldingResources.iterator();
					while(myiterator.hasNext())
					{
						ThreadState Resource=myiterator.next().resource;
						int max =0;
						for(int i=0; i<Resource.waitingthreadstates.size();i++)
						{
							if (max < Resource.waitingthreadstates.get(i).effectivePriority)
								max = Resource.waitingthreadstates.get(i).effectivePriority;

						}
						if (Resource.priority >= max) Resource.effectivePriority = Resource.priority;
		...
		public void waitForAccess(PriorityQueue waitQueue) {
			...
			if(waitQueue.transferPriority==true)
			{
				...
				{
					waitQueue.resource.waitingthreadstates.add(this);
					ChangePriority();
				}
